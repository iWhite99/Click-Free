...
char miscari[500]="";
int ok1=0,ok2=0;
int ok11=0,ok22=0,ok33=0,ok44=0;
...
cv::Vec6d visualise_tracking(cv::Mat& captured_image, const LandmarkDetector::CLNF& face_model, const LandmarkDetector::FaceModelParameters& det_parameters, cv::Point3f gazeDirection0, cv::Point3f gazeDirection1, int frame_count, double fx, double fy, double cx, double cy)
{
	...
	if (detection_certainty < visualisation_boundary)
	{
		...
		if(pose_estimate_to_draw[3] < -0.3){
			...
			{
				strcat(miscari,"jos, ");
				//cout<<miscari<<endl<<"HAFAWAHDAWDHA"<<endl;
				ok1=1;
			}
			ok11=1;
		}
		...
		// 	cout << "Z_pos "<<endl;
		// 	cout<<miscari<<endl;
		// 	send_message(5000, "127.0.0.1", "http://127.0.0.1:5000/detectPosition", miscari);
		// }
		cout <<miscari<<endl;
		/*
		if(X_neg && X_pos && Y_neg && Y_pos && Z_pos && Z_neg && (i < 500)){
			send_message(5000, "127.0.0.1", "http://127.0.0.1:5000/detectPosition", "Is real");
			X_neg = false;
			X_pos = false;
			Y_neg = false;
			Y_pos = false;
		...
		*/
		...
		if(ok11==1&&ok22==1&&ok33==1&&ok44==1)
			send_message(5555,"10.0.0.3", "http://10.5.5.39/detectPosition","Is Real");
...
{ 
	...
	if (sockfdc < 0) 
	error("ERROR opening CLIENT socket");
	serverc = gethostbyname("10.0.0.3");
	if (serverc == NULL) {
	fprintf(stderr,"ERROR, no such host CLIENT\n");
	exit(0);
	}
	...
	while (1){
		...
		int retursStatus=200;
		write(newsockfd,(char *)&retursStatus,sizeof(size));
